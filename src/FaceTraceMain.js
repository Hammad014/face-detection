...
const MainPage = () => {
  ...
  useEffect(() => {
    ...
    // Add null check for mountRef.current
    if (mountRef.current) {
  ...
  const toggleDropdown = (step) => {
    setOpenDropdowns(prev => ({ ...prev, [step]: !prev[step] }));
  };
          ...
          <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
            <div className="flex justify-between items-center h-32">
              ...
              <div className="md:hidden">
                ...
                >
                  {isNavOpen ? <FaTimes className="h-6 w-6" /> : <FaBars className="h-6 w-6" />}
                </button>
              </div>
              
              <nav className={`md:flex md:items-center ${isNavOpen ? 'block' : 'hidden'} absolute md:static bg-gray-900/90 md:bg-transparent top-32 left-0 w-full md:w-auto`}>
                <ul className={`flex flex-col items-center md:flex-row md:space-x-4 ${isNavOpen ? 'space-y-6 py-4' : ''}`}>
                  <li>
                    <button
        ...
        <div className="bg-transparent text-white z-10 p-8 mt-3">
          <div className="mb-10 text-center">
            ...
            <p className="mt-6 text-xl text-gray-300 max-w-2xl mx-auto font-light">
              Revolutionizing campus security through AI-powered facial recognition technology
            </p>
          </div>

          {/* Core Features - Reverted to old structure */}
          <div className="mt-20 px-5">
            <h2 className="text-2xl font-bold mb-8 text-center text-white">Core Features of FaceTrace</h2>
            <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-8 justify-items-center">
              <div className="bg-gray-800 p-6 rounded-lg shadow-xl text-center hover:scale-105 transition-transform duration-300 border border-transparent hover:border-blue-500">
                ...
                <p>Complete registration of students, faculty, and workers with secure data handling.</p>
              </div>
              ...
              <div className="bg-gray-800 p-6 rounded-lg shadow-xl text-center hover:scale-105 transition-transform duration-300 border border-transparent hover:border-blue-500">
                ...
                <p>Identification of individuals through advanced facial recognition system.</p>
              </div>
              ...
              <div className="bg-gray-800 p-6 rounded-lg shadow-xl text-center hover:scale-105 transition-transform duration-300 border border-transparent hover:border-blue-500">
                ...
                <p>Automated alerts after identification of known and unknown persons.</p>
              </div>
              ...
              <div className="bg-gray-800 p-6 rounded-lg shadow-xl text-center hover:scale-105 transition-transform duration-300 border border-transparent hover:border-blue-500">
                ...
                <p>Notifications for administrator devices to take action on unknown person detection.</p>
              </div>
            </div>
          </div>
          ...
          <div id="how-it-works" className="mt-28">
            ...
            <div className="max-w-6xl mx-auto p-8 space-y-8">
              <div className="grid md:grid-cols-4 gap-8">
                {[
                  { step: 'step1', icon: <FaUserShield />, title: "Secure Login System", color: 'cyan' },
                  { step: 'step2', icon: <FaDesktop />, title: "Dashboard Access", color: 'blue' },
                  { step: 'step3', icon: <FaUserPlus />, title: "Member Registration", color: 'purple' },
                  { step: 'step4', icon: <FaGavel />, title: "AI Model Operation", color: 'pink' },
                ].map((stepInfo, idx) => (
                  <div key={idx} className="bg-gray-900/50 p-6 rounded-xl border border-gray-700/30 hover:border-cyan-500/30 transition-all duration-300">
                    <div className={`text-${stepInfo.color}-400 mx-auto mb-4 text-4xl`}>{stepInfo.icon}</div>
                    <h3 className="text-xl font-semibold mb-4">{stepInfo.title}</h3>
                    <button
                      onClick={() => toggleDropdown(stepInfo.step)}
                      className={`hover:bg-${stepInfo.color}-700/40 bg-blue-500 text-white px-4 py-2 rounded-md transition-all duration-200`}
                    >
                      {openDropdowns[stepInfo.step] ? 'Hide Details' : 'Learn More'}
                    </button>
                    <div className={`mt-4 transition-all duration-300 ${openDropdowns[stepInfo.step] ? 'opacity-100' : 'opacity-0 h-0'}`}>
                      <p className="text-gray-400 font-light">
                        {[
                          "Advanced security protocols with multi-factor authentication",
                          "Customizable dashboard with real-time analytics",
                          "Biometric data collection and secure storage",
                          "Deep learning model with continuous training"
                        ][idx]}
                      </p>
                    </div>
                  </div>
          ...
          {/* About Section - Old content with new styling */}
          <div id="about-face-trace" className="mt-28 px-4 md:px-8 lg:px-16">
            ...
            <div className="max-w-6xl mx-auto text-gray-300 space-y-6">
              <p className="bg-gray-900/50 p-6 rounded-xl border border-gray-700/30 backdrop-blur-sm">
                FaceTrace is a cutting-edge facial recognition system tailored exclusively for Comsats University Abbottabad. By leveraging state-of-the-art algorithms, FaceTrace ensures robust campus security, accurately identifying and monitoring individuals in real-time.
              </p>
              <p className="bg-gray-900/50 p-6 rounded-xl border border-gray-700/30 backdrop-blur-sm">
                The system maintains a comprehensive database of authorized personnel, including students, faculty, and staff, ensuring that only registered individuals can access campus facilities. FaceTrace proactively detects and alerts security personnel about any unauthorized access attempts, enhancing overall campus safety.
              </p>
              <p className="bg-gray-900/50 p-6 rounded-xl border border-gray-700/30 backdrop-blur-sm">
                Beyond security, FaceTrace streamlines administrative tasks such as attendance management and disciplinary case handling. The seamless integration of these features ensures data integrity and operational efficiency, allowing administrators to focus on enhancing the educational environment.
              </p>
              <p className="bg-gray-900/50 p-6 rounded-xl border border-gray-700/30 backdrop-blur-sm">
                Continuous updates and secure data handling mechanisms ensure that FaceTrace remains reliable and effective in maintaining the integrity of campus operations. With its user-friendly interface and real-time capabilities, FaceTrace stands as a vital tool in modernizing campus security and administration.
              </p>
            </div>
          </div>
        </div>
